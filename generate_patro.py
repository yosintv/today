...
def get_html_template(target_day, month_days, month_label, ad_month):
    ...
    for day in month_days:
        ...
        # This keeps URLs at the root: domain.com/2082-09-17.html
...
<body class="bg-slate-50 text-slate-900 antialiased">
    ...
    <main class="max-w-4xl mx-auto px-4">
        ...
        <section class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 mb-10">
            <h3 class="text-2xl font-black text-slate-800 mb-8 uppercase tracking-tight">{month_label} / {ad_month}</h3>
...
def build_site():
    ...
    for month_data in data.get('calendar_data', []):
        m_label = " / ".join(month_data.get('bs_months', []))
        m_ad = month_data.get('month', '')
        
        for day in month_data.get('days', []):
            html = get_html_template(day, month_data['days'], m_label, m_ad)
            
            # Save at root for short URLs
            ...
            # Update index.html for current day
            if day['ad'] == TODAY_AD:
