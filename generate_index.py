...
def get_html_layout(target_day, month_days, today_ad, month_label):
    """Generates the full HTML with your exact CSS design."""
    ...
    for day in month_days:
        # If this is the specific page for this day, highlight it
    ...
    <main class="max-w-4xl mx-auto px-4">
        ...
        <section class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 mb-10">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <h3 class="text-2xl font-bold text-slate-800 uppercase tracking-tight">{month_label}</h3>
...
def run():
    ...
    # Iterate through months in 2026.json
    for month_block in full_data['calendar_data']:
        month_label = " / ".join(month_block['bs_months'])
        
        for day in month_block['days']:
            all_days_collected.append(day)
            html_output = get_html_layout(day, month_block['days'], today_ad, month_label)
            
            # 1. CREATE THE INDIVIDUAL PAGE (e.g., 2082-09-28.html)
            filename = f"{day['bs']}.html"
            with open(filename, "w", encoding="utf-8") as f:
                f.write(html_output)
            
            # 2. IF IT IS TODAY, ALSO CREATE index.html
            if day['ad'] == today_ad:
    ...
    print(f"Success: Created {len(all_days_collected)} day pages, index.html, and sitemap.xml")
